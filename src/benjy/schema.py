...
class Schema:
    ...
    def column_names(self, source):
        return {obj['origin']: obj['name'] for obj in self.columns if 'entity' not in obj}
    ...
    def execute(self):
        ...
        data = ((self.driver.load(self.source_refs.get(source_id)), source_id) for source_id in self.sources)
        data = ((self.driver.select(table, self.needed_columns(source_id), self.column_names(source_id)), source_id)
                for table, source_id in data)

        if len(entities) > 0:
            crosswalker = crosswalk_entities(self.driver, self.source_refs, self.entity_graph, entities)
            data = ((crosswalker(table, source_id), source_id) for table, source_id in data)
